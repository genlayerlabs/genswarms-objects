...
check.("normalize: run_at shapes (map, bare string, legacy keys)",
...
check.("normalize: every_ms and cron kinds",
  Schedule.normalize(%{"every_ms" => 300_000}, now) ==
    {:ok, %{"kind" => "every_ms", "every_ms" => 300_000}} and
  Schedule.normalize(%{"cron" => "0 * * * *"}, now) ==
    {:ok, %{"kind" => "cron", "expr" => "0 * * * *"}})

check.("normalize rejects: bad every_ms, invalid expr, UNSATISFIABLE expr, junk",
...
check.("first fire: every_ms = created + period, never immediate",
  Schedule.first_run_at(every5, now) == {:ok, now + 300_000})

check.("first fire: cron = next match after created",
  Schedule.first_run_at(hourly, now) == {:ok, ms.("2026-07-06T15:00:00Z")})

check.("grid rule: normal cadence advances one period",
  Schedule.next_after(every5, now, now + 1_000) == {:ok, now + 300_000})

check.("grid rule: downtime of 3.5 periods → EXACTLY ONE catch-up, next strictly future",
  Schedule.next_after(every5, now, now + 1_050_000) == {:ok, now + 1_200_000})

check.("grid rule REGRESSION: never max(now, due+period) double-fire",
  (fn -> {:ok, n} = Schedule.next_after(every5, now, now + 1_050_000); n > now + 1_050_000 end).())

check.("grid rule: cron next is after max(due, now)",
  Schedule.next_after(hourly, ms.("2026-07-06T15:00:00Z"), ms.("2026-07-06T18:30:00Z")) ==
    {:ok, ms.("2026-07-06T19:00:00Z")})

check.("one-shots have no next; recurring? flags kinds",
